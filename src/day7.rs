...
fn parse_antecedent<'a>(s: &'a str) -> Ant<'a> {
    ...
    match s.parse() {
	Ok(l) => return Ant::Ref(Ref::Lit(l)),
	Err(_) => return Ant::Ref(Ref::Var(s)),
    }
}
...
fn run_to_a<'a>(state: &mut HashMap<&'a str, Option<u16>>, rules: &[Rule<'a>], override_b: Option<u16>) {
	...
	for rule in rules {
		...
		Ant::Or(l, r) => {
		    match (deref(l, state), deref(r, state)) {
